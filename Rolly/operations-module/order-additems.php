...
<div class="btn-box">
  ...
  ?>
<?php if($user-> get_user_access($user_id) == "Owner" OR $user-> get_user_access($user_id) == "Supervisor") {?> 
...
<a class="btn-jsactive" onclick="document.getElementById('id03').style.display='block'">Delete</a> <?php } else {} ?> 
      <?php
    }
    ?>
</div>
...
<form method="POST" id="itemDelete" action="processes/process.release.php?action=delete">
      <input type="hidden" id="orderid" name="orderid" value="<?php echo $id;?>"/>
      </form>
      <div #id="form-update" class="modal-content">
    <div class="container">
      ...
      <div class="clearfix">
         <a href="processes/process.release.php?action=delete&id=<?php echo $id;?>" class="btn-jsactive">Delete</a>
         <a class="cancelbtn" onclick="document.getElementById('id03').style.display='none'">Cancel</a>
      </div>
    </div>
    </div>
...
var modal_delete = document.getElementById('id03');
